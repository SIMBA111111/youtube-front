export const getShortPlaylistsByChannelHash = (channelHash?: string) => {
    ...
    // Если передан hash канала, фильтруем плейлисты по нему
    if (channelHash) {
        return mockPlaylists.filter(playlist => playlist.channel.hash === channelHash);
    }
