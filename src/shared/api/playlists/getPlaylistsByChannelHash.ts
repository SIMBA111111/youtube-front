...
export const getPlaylistsByUsername = async (channelUsername: string, limit: number = 20, offset: number = 0) => {
    console.log('getPlaylistsByUsername');
    
    try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/playlists/by-username/${channelUsername}?limit=${limit}&offset=${offset}`)
        ...
        new Error(`Error getPlaylistsByUsername: ${error}`);
        return []
    }
};